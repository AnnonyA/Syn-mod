...
- bounded append batching with progressive source-chunk release on the `AlternativeWritefile` path
- a compact validation snapshot so structural validation does not keep the full chunk list populated throughout file writing
...
saveinstance({
    ...
    MetricsCallback = function(report)
        ...
        print(report.counters.peakBufferedBytes)
    end,
})
```

The report includes stage timings for collection, serialization, compression, output joining, writing, and aggregate decompilation time; collected-instance counts; property read/serialization counters; compression attempt/use/rejection counters and byte totals; output-join counts/bytes; streaming write observations; decompiler activity; output bytes; recovery skips; the selected compatibility profile; the latest validation report when validation is enabled; and `gcinfo()` start/end/delta values when that API is available.

Memory-related values have deliberately narrow meanings. `gcinfo()` start/end/delta values are observations, not process peak-memory measurements. `peakBufferedBytes` is the largest write buffer/payload observed by the Memory/Streaming v1 path; it is also **not** total process peak memory.

## Memory/Streaming v1

When `AlternativeWritefile` is enabled and a working `appendfile` capability is available, Syn-mod writes using bounded append batches instead of concatenating the entire output into one string. Memory/Streaming v1 tightens this path by flushing the pending batch before another small chunk would push it past the configured segment threshold and by releasing original chunk references progressively after their bytes are retained by a pending batch or written as a large-chunk segment.

Structural validation no longer requires those original strings to stay in the chunk table until validation runs. Before writing, Syn-mod records a compact validation snapshot containing the generated chunk count, total generated bytes, non-empty state, and Binary/XML envelope result. The normal validation file-existence/read-back checks still run after writing.

The fallback `writefile` path still has to build a whole-file string because only one write call is available there, but it clears the source chunk table immediately after the join so those references do not need to remain live during the final write call.

This is **bounded write/output streaming, not a fully streaming serializer**. Binary/XML serialization still produces the chunk list before the write phase begins. The new Metrics counters (`streamFlushes`, `streamedBytes`, `peakBufferedBytes`, and `chunksReleased`) make this behavior observable, but Syn-mod does not claim lower total process peak RAM until real executor measurements demonstrate it.
...
- `patches/memory-streaming-v1.patch` — bounded append buffering, progressive chunk release, validation snapshots, and write-buffer metrics.
...
- `tests/test_memory_streaming.py` — Memory/Streaming v1 regression contract.
